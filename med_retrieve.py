...
collection_airbnb = db.MedDB.MedCollection
...
results = collection_airbnb.aggregate([
  {
    "$vectorSearch": {
      ...
      "index": "med2_index", # the index name you used in Step 4.
    }
  }
])
results_as_dict = {doc['Completion']: doc for doc in results}

print(f"From your query \"{query}\", the following diagnosis was found:\n")
print("\n".join([str(i+1) + ". " + name for (i, name) in enumerate(results_as_dict.keys())]))

your_task_prompt = (
    "From the given medical diagnosis, choose the most likely dianosis. "
)
...
for doc in results_as_dict.values():
  listing_data += f"Diagnosis: {doc['name']}\n"
...
messages = [
    {
        "role": "system",
        "content": f"Diagnosis data: {listing_data}"
    },
    {
        "role": "user",
        "content": f"{query}"
